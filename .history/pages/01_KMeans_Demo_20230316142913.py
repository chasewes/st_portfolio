...
centroids = np.append(centroids, np.random.randint(np.min(data), np.max(data), size=(k-4,2)), axis=0
...
fig, ax = plt.subplots()
ax.scatter(data[:,0], data[:,1])
#add the centroids to the plot, give each centroid a different color, make the centroid points x's
