...
st.write("first, we will randomly select 4 points to be the initial centroids")

#centroids should be between the min and 
centroids = data[np.random.randint(min(data[0,:]),max(data[1,:]),4),:]
st.write("Here are the initial centroids")
st.write(centroids)
